...
\end{proof}
\begin{bemerkung}
	Im Vergleich zu~\cite[Lemma~5]{PaesLeme2018} ist in der Abschätzung hier aufgrund der unterschiedlichen LP-Darstellung von~\eqref{DP} die Anzahl der Käufer $m$ enthalten.
\end{bemerkung}
...
\end{theorem}
\begin{proof}
	Seien zunächst $M'\coloneqq n^{\bigO(1)}$ und $K\coloneqq \arg\min_{\fp\in\R^n} f(\fp)\cap [-M', M']^n$ die konvexe, nichtleere Menge der Minimierer von $f$ in $[-M',M']^n$.
