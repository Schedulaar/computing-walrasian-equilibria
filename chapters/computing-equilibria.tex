...
Hier ist unklar, wie das Zitat aus dem ursprünglichen Theorem folgt, jedoch haben sich wohl zumindest kleinere Fehler eingeschlichen:
So ist einerseits nicht klar, wie Leme und Wong auf die Abschätzung $\bigO(n\delta \log(Mn/\delta))$ für die Breite von $P$ kommen, da man nach Lee u.a. eher $\bigO(n\delta \log(M/\delta))$ erwarten würde.
Weiterhin sind die Folgerungen über die Konvexkombination jeweils unterschiedlich formuliert:
So betrifft diese bei Lee u.a. nur die Vektoren $a_2,\dots, a_{\bigO(n)}$ und $a_1$ spielt hier eine gesonderte Rolle.
Bei Paes und Leme ist diese gesonderte Rolle nicht mehr vorhanden und es wird über eine Konvexkombination $\sum_{i\in[\bigO(1)]} t_i$ geredet.
Außerdem fehlen wohl in den Abschätzungen der Konvexkombinationen bei Paes und Leme jeweils die Vorfaktoren $t_i$, sodass diese wohl die folgenden Abschätzungen meinen:
\[
...
\]
Des Weiteren nutzen Paes und Leme in \cite[Theorem~6]{PaesLeme2018} das obige Ergebnis, um konvexe Funktionen mit einem Subgradienten-Orakel zu minimieren.
Im einfacheren Fall, in dem alle Restriktionen des Polytops dem Orakel entspringen, wählt man $\delta$ so klein, dass bereits $\varepsilon=\bigO(n\delta L \log(M'/ \delta))$ gilt.
Wegen $\lim_{\delta \rightarrow 0}\delta \cdot \log(\delta)= 0$ ist dies möglich, jedoch gehen Paes und Leme nicht darauf ein, wieso dadurch keine negativen Auswirkungen auf die Laufzeit entstehen: Je kleiner $\delta$ gewählt wird, desto länger ist auch die Laufzeit nach~\cite[Theorem~5]{PaesLeme2018}.
